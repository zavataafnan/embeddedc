/**********************************************************************
* $Id: debug_frmwrk.c 28 2014-01-27 14:45:13Z danielru $		debug_frmwrk.c			2011-06-02
*//**
* @file		debug_frmwrk.c
...
uint8_t UARTGetChar (UART_ID_Type UartID)
{
	...
	UART_Receive(UartID, &tmp, 1, BLOCKING, 500);
...
{	
    if(c == NULL)
        return FALSE;		
    if(UART_Receive(UartID, c, 1, NONE_BLOCKING, 500))
        return TRUE;	
    return FALSE;
}
												...
												uint8_t numCh, uint32_t* val)
{
	...
	while (numCh > 0)
	{
		...
		UART_Receive(UartID, &tmpCh, 1, BLOCKING, 500);
