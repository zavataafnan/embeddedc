/**********************************************************************
* $Id: lpc177x_8x_uart.c 28 2014-01-27 14:45:13Z danielru $		lpc177x_8x_uart.c			2011-06-02
*//**
...
uint32_t UART_Send_WithReflect(UART_ID_Type UartID, uint8_t *txbuf,
                   uint32_t buflen, TRANSFER_BLOCK_Type flag)
{
	uint8_t dummy;
  uint32_t bToSend, bSent, timeOut, fifo_cnt;
  ...
  if (flag == BLOCKING)
  {
    ...
    while (bToSend)
    {
      ...
      while (fifo_cnt && bToSend)
      {
        ...
        //uint8_t rLen = UART_Receive(UartID, &dummy, 1, NONE_BLOCKING, 5);
        //dummy = UART_ReceiveByte(UartID);
...
uint32_t UART_Receive(UART_ID_Type UartID, uint8_t *rxbuf,
                      uint32_t buflen, TRANSFER_BLOCK_Type flag, uint32_t tout)
{
...
//////
void ResetFIFOBuffer(UART_ID_Type UartID)
{
  ...
  case UART_2:
    ...
    while (( (LPC_UART2)->LSR & (UART_LSR_THRE|UART_LSR_TEMT)) != (UART_LSR_THRE|UART_LSR_TEMT) );
    while ( (LPC_UART2)->LSR & UART_LSR_RDR )
    {
      LSR = (LPC_UART2)->RBR;	/* Dump data from RX FIFO */
    }
