...
uint32_t SDRAM_BASE_ADDR  = 0xA0000000;
...
uint32_t sdram_test( void )
{
  ...
  /* 16 bit write */
  for (i = 0; i < SDRAM_SIZE/divider1; i++)
  {
    for (j = 0; j < 0x100; j++)
    {
      *short_wr_ptr++ = (i + j);
...
uint32_t sdram_init (void)
{
  ...
  //power on external ram module
  LPC_SC->PCONP     |= 0x00000800;
  //delay control register settings
  //2.5ns EMC (external memory controller) clock delay
  LPC_SC->EMCDLYCTL   = 0x00001010;
  //EMC control register
  //EMC module enable
  LPC_EMC->Control   = 0x00000001;
  //EMC configuration register default
  ...
  LPC_EMC->DynamicConfig0    = 0x00004300; 
#else
...
//Timing for 50MHz Bus (with 100MHz M3 Core)
...
#ifdef SDRAM64
  //for 64Mbit chip  ///0x75
  LPC_EMC->DynamicRefresh    = 0x0000003A; /* ( n * 16 ) -> 1872 clock cycles -> 31.2uS at 60MHz <= 31.25uS ( 64ms / 2048 row ) */
#else
    //for 256Mbit chip
  ...
  //Timing for 48/60/72MHZ Bus (see page 178 for calculating this value)
#ifdef SDRAM64
  dwtemp = *((volatile uint32_t *)(SDRAM_BASE | (0x22<<(2+2+8)))); /* 4 burst, 2 CAS latency */
	#else
...
//[re]enable buffers
#ifdef SDRAM64
  LPC_EMC->DynamicConfig0    = 0x00084300;   /* 64MB, 2Mx32, 4 banks, row=11, column=8 */
#else
...
unsigned char sdram_test_all(void)
{
