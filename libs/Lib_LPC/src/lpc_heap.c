...
//EA MODIFICATION - THIS BLOCK WAS ONLY IN THE "ELSE" BLOCK AND NOT THE "IF"
    /* Set return address past the heap descriptor */
    return_address = (UNS_32) insert_ptr + HEAP_HEAD_SIZE;
//EA MODIFICATION
...
/***********************************************************************
 *
 * Function: lpc_get_allocatedsize
 *
 * Purpose: Returns the number of allocated bytes.
 *
 * Processing:
 *     See function.
 *
 ...
 * Outputs: None
 *
 * Returns: The size of the used heap area in bytes.
 *
 * Notes: None
 *
 **********************************************************************/
UNS_32 lpc_get_allocatedsize(void)
{
...
/***********************************************************************
 *
 * Function: lpc_reallocate
 *
 * Purpose: Reallocate an area on the heap. If the area is moved
 *          then the data will be preserved up to the smallest of
 ...
 *
 * Parameters:
 *     current_addr  : Address of allocated entry
 *     size_in_bytes : Byte size of the requested allocation chunk
 *
 * Outputs: None
 *
 ...
 *
 **********************************************************************/
void *lpc_realloc(void *current_addr, UNS_32 size_in_bytes)
{
  void* new_addr = NULL;
  HEAP_DESCRIPTOR_T *current_ptr;

  if (size_in_bytes == 0)
  {
    lpc_free(current_addr);
    return NULL;
  }
  if (current_addr == NULL)
  {
    return lpc_new(size_in_bytes);
  }

  current_ptr = lpc_find_entry(current_addr);
  if (current_ptr != HEAP_POINTER_NULL)
  {
    if (size_in_bytes == current_ptr->entry_size) {
      return current_addr;
    }

    new_addr = lpc_new(size_in_bytes);
    if (new_addr != NULL)
    {
      if (size_in_bytes > current_ptr->entry_size)
      {
        memcpy(new_addr, current_addr, current_ptr->entry_size);
      }
      else
      {
        memcpy(new_addr, current_addr, size_in_bytes);
      }
    }
    lpc_remove_entry(current_addr);
  }
  return new_addr;
}
