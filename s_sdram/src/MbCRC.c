...
{
	USHORT crc_f = CalcModBusCRC16(pucFrame, usLen);
	pucFrame[usLen+0] = crc_f & 0xff;
	pucFrame[usLen+1] = crc_f >> 8;
}
...
void CalcCorusCRC16(uint8_t * ayData, uint16_t nLen, uint8_t * ayCRC)
{
	...
	for (i = 0; i < nLen; i++)
	{
		current = (uint16_t)(ayData[i] << 8);
		for (j = 0; j < 8; j++)
		{
			if ((short)(crc_reg ^ current) < 0)
				crc_reg = (uint16_t)((crc_reg << 1) ^ 0x8005);
			else
				crc_reg <<= 1;
			current <<= 1;
		}
	...
	ayCRC[0] = (uint8_t)(crc_reg & 0xFF);
	ayCRC[1] = (uint8_t)((crc_reg >> 8) & 0xFF);
}
